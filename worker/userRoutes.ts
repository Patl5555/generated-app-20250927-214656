...
export function userRoutes(app: Hono<{ Bindings: Env }>) {
    ...
    app.get('/api/mcp/config', async (c) => {
        try {
            ...
            return c.json({ success: false, error: 'Failed to retrieve configuration' }, { status: 500 });
        }
    });
    app.put('/api/mcp/config', async (c) => {
        try {
            ...
            const controller = getAppController(c.env);
            await controller.updateConfiguration(body);
            return c.json({ success: true, data: { message: 'Configuration saved' } });
        } catch (error) {
            console.error('Failed to save MCP config:', error);
            return c.json({ success: false, error: 'Failed to save configuration' }, { status: 500 });
        }
    });
